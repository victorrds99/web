...
function App() {
  ...
  useEffect(() => {
    fetch('http://localhost:3334/games').then(response => response.json())
    .then(data => {
      setGames(data)
    })
  },[])
              ...
              <form className='mt-8 flex flex-col gap-4'>
                ...
                <div className='grid grid-cols-2 gap-6'>
                  <div className='flex flex-col gap-2'>
                    ...
                    <Input id='yearsPlaying' type='number' placeholder='tudo bem começar hoje' />
                  </div>
                  <div className='flex flex-col gap-2'>
                    <label htmlFor="discord">Qual seu discord?</label>
                    <Input id='discord' type='text' placeholder='Usuario8888'/>
                  </div>
                </div>
